...
# def batch_normalize(x, B, W, H, C):
#     mean_list = []
#     std_list = []
#     mean = torch.mean(x ** , axis=(0, 1, 2), keepdims=True)
#     var = torch.var(x, axis=(0, 1, 2), keepdims=True)
#     mean = np.mean(x, a)
#     for c in range(C):
#         mean_sum = 0
#         for n in range(B):
...
class BN_ConvolutionalProcessingBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(BN_ConvolutionalProcessingBlock, self).__init__()
    ...
    def build_module(self):
        ...
        self.layer_dict['bn_0'] = nn.BatchNorm2d(num_features=self.input_shape[1])
        ...
        self.layer_dict['bn_1'] = nn.BatchNorm2d(num_features=self.input_shape[1])
...
class BN_ConvolutionalDimensionalityReductionBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(BN_ConvolutionalDimensionalityReductionBlock, self).__init__()
