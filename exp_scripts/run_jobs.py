...
while total_jobs_finished < args.total_epochs * len(list_of_scripts):
    ...
    with tqdm.tqdm(total=len(list_of_scripts)) as pbar_experiment:
        while curr_idx < len(list_of_scripts):
            number_of_jobs = 0
            for experiment in list_of_scripts:
                if check_if_experiment_with_name_is_running(experiment_name=experiment.decode("utf-8")):
                    number_of_jobs += 1
            ...
            if number_of_jobs < args.num_parallel_jobs:
                ...
                str_to_run = 'sbatch -o {}_{}.out {} '.format(list_of_scripts[curr_idx].decode("utf-8")[:-3], str(total_jobs_finished + 1), list_of_scripts[curr_idx].decode("utf-8"))
